...
public class MatrixManager {
	...
	public void createFiles(ArrayList<String> path_list) throws FileNotFoundException {
		...
		for (String repo : path_list) {
			...
			if (files.contains(repoName + ".txt")) {
				...
				repository_object = navigator.Files_List(file, repository_object.getMainList(), terms,
						repository_object);
	...
	public RealMatrix cleanMatrix(RealMatrix m) {
		...
		/*
		 * prendiamo la matrice realmatrix termini-documenti ed eliminiamo alcune
		 * colonne secondo questo principio: se un termine appare in una sola repository
		 * lo eliminiamo oppure se appare in more than half delle repository
		 */
